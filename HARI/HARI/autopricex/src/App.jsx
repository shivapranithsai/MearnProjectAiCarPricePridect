...
const Predict = () => {
  ...
  const handlePredict = async (e) => {
    ...
    try {
      ...
      setPrediction(`₹${Number(predicted_price).toLocaleString()}`);
    } catch (err) {
      console.error(err);
      setPrediction("Error calculating price. Please try again.");
    } finally {
      setLoading(false);
    }
  };
...
const Sell = () => {
  const navigate = useNavigate();
  const [formData, setFormData] = useState({ 
    brand: "", model: "", year: "", mileage: "", condition: "", fuelType: "", price: "", description: "", features: [], 
    pics: null, picPreview: null, name: "", phone: "", location: "" 
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        const { error } = await response.json();
        throw new Error(error || "Listing failed");
      }
      ...
      <div style={formContainer}>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} style={formStyle}>
            ...
            <input type="number" placeholder="� Year" value={formData.year} onChange={(e) => setFormData({...formData, year: e.target.value})} style={inputStyle} required />
            <input type="number" placeholder="🛣️ Mileage (km)" value={formData.mileage} onChange={(e) => setFormData({...formData, mileage: e.target.value})} style={inputStyle} required />
            ...
            {/* 🔥 CONTACT DETAILS */}
            <input placeholder="👤 Your Name" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} style={inputStyle} required />
            <input type="tel" placeholder="📞 Phone Number" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} style={inputStyle} required />
            <input placeholder="📍 Location (City, State)" value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} style={inputStyle} required />
    ...
    <div style={pageStyle}>
      <div style={formContainer}>
        ...
        <form onSubmit={handleSubmit} style={formStyle}>
          ...
          <button style={predictBtn} type="submit" disabled={status === "loading"}>
            {status === "loading" ? "Creating account..." : "Sign Up"}
          </button>
          {status === "success" && <p style={{ color: "#34d399", textAlign: "center" }}>Registration successful! Redirecting to login...</p>}
        </form>
        <p style={{ textAlign: "center", color: "rgba(255,255,255,0.6)", marginTop: "15px" }}>
...
const Buy = () => {
  ...
  const [error, setError] = useState("");
  const [buyDetails, setBuyDetails] = useState({ name: "", email: "", phone: "", message: "" });
  const [buyStatus, setBuyStatus] = useState("");

  const clearBuyForm = () => setBuyDetails({ name: "", email: "", phone: "", message: "" });

  const sendBuyRequest = async (e) => {
    e.preventDefault();
    if (!selectedListing) return;

    if (!buyDetails.name || !buyDetails.email || !buyDetails.phone) {
      setBuyStatus("error");
      alert("Please fill name, email, and phone before sending a buy request.");
      return;
    }
    ...
    try {
      const payload = {
        listing_id: selectedListing.id || selectedListing._id || null,
        listing: selectedListing,
        name: buyDetails.name,
        email: buyDetails.email,
        phone: buyDetails.phone.startsWith("+") ? buyDetails.phone : "+91" + buyDetails.phone,
        message: buyDetails.message,
      };

      const response = await fetch("http://localhost:5000/api/buy_requests", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to send buy request");
      }

      setBuyStatus("success");
      clearBuyForm();
      setTimeout(() => setBuyStatus(""), 3500);
    } catch (err) {
      console.error(err);
      setBuyStatus("error");
      alert(err.message || "Buy request failed");
    }
  };

  const sendWhatsAppMessage = () => {
    if (!selectedListing || !selectedListing.phone) return;
    const message = `Hi, I'm interested in your ${selectedListing.brand} ${selectedListing.model}. My name is ${buyDetails.name}, email: ${buyDetails.email}, phone: ${buyDetails.phone}. ${buyDetails.message}`;
    const whatsappUrl = `https://wa.me/${selectedListing.phone.replace('+', '')}?text=${encodeURIComponent(message)}`;
  ...
  return (
    <div style={pageStyle}>
      <div style={{ ...formContainer, maxWidth: "900px" }}>
        <h1 style={titleStyle}>🛒 Browse Cars</h1>
        
        {loading && <p style={{ textAlign: "center", color: "#60a5fa" }}>Loading listings...</p>}
        {error && <p style={{ textAlign: "center", color: "#f87171" }}>❌ {error}</p>}
        
        {!loading && listings.length === 0 && (
          <p style={{ textAlign: "center", color: "rgba(255,255,255,0.8)", fontSize: "18px" }}>
            No cars listed yet. Check back later! 🔥
          </p>
        )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "20px", marginBottom: "20px" }}>
            {listings.map((listing, idx) => (
              <div key={idx} style={{
                ...
                <p style={{ margin: "8px 0" }}>💰 <strong>₹{Number(listing.price).toLocaleString()}</strong></p>
                {listing.description && <p style={{ fontSize: "14px", opacity: "0.8", margin: "8px 0" }}>{listing.description}</p>}
                {listing.year && (
                ...
                {listing.mileage && (
                  <p style={{ fontSize: "14px", margin: "6px 0" }}>
                    🛣️ Mileage: {listing.mileage.toLocaleString()} km
                  </p>
                )}
                {listing.condition && (
                  <p style={{ fontSize: "14px", margin: "6px 0" }}>
                ...
                {/* Contact info is hidden here; visible only in modal when 'Contact Seller' is clicked */}
                <button onClick={() => setSelectedListing(listing)} style={{
                  ...predictBtn,
      ...
      {selectedListing && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ marginBottom: "14px", color: "#60a5fa" }}>📬 Seller Details</h2>
            <p style={{margin: "8px 0"}}><strong>Car:</strong> {selectedListing.brand} {selectedListing.model}</p>
            {selectedListing.year && <p style={{margin: "8px 0"}}><strong>Year:</strong> {selectedListing.year}</p>}
            {selectedListing.mileage && <p style={{margin: "8px 0"}}><strong>Mileage:</strong> {Number(selectedListing.mileage).toLocaleString()} km</p>}
            {selectedListing.condition && <p style={{margin: "8px 0"}}><strong>Condition:</strong> {selectedListing.condition}</p>}
            {selectedListing.fuelType && <p style={{margin: "8px 0"}}><strong>Fuel Type:</strong> {selectedListing.fuelType}</p>}
            {selectedListing.price && <p style={{margin: "8px 0"}}><strong>Price:</strong> ₹{Number(selectedListing.price).toLocaleString()}</p>}
            <hr style={{borderColor: "rgba(255,255,255,0.2)", margin: "14px 0"}}/>
            <p style={{margin: "8px 0"}}><strong>Seller:</strong> {selectedListing.name || "N/A"}</p>
            <p style={{margin: "8px 0"}}><strong>Phone:</strong> {selectedListing.phone || "N/A"}</p>
            <p style={{margin: "8px 0"}}><strong>Location:</strong> {selectedListing.location || "N/A"}</p>
            {selectedListing.description && <p style={{margin: "8px 0", fontStyle: "italic", opacity: 0.9}}><strong>Notes:</strong> {selectedListing.description}</p>}
            <hr style={{borderColor: "rgba(255,255,255,0.2)", margin: "14px 0"}} />
            <h3 style={{ color: "#60a5fa", marginBottom: "8px" }}>📩 Send Buy Request</h3>
            <form onSubmit={sendBuyRequest} style={{ display: "grid", gap: "10px" }}>
              <input
                style={inputStyle}
                type="text"
                placeholder="Your name"
                value={buyDetails.name}
                onChange={(e) => setBuyDetails({ ...buyDetails, name: e.target.value })}
                required
              />
              <input
                style={inputStyle}
                ...
                value={buyDetails.email}
                onChange={(e) => setBuyDetails({ ...buyDetails, email: e.target.value })}
                required
              />
              <input
                style={inputStyle}
                ...
                value={buyDetails.phone}
                onChange={(e) => setBuyDetails({ ...buyDetails, phone: e.target.value })}
                required
              />
              <textarea
                style={{ ...inputStyle, height: "60px" }}
                placeholder="Message for seller (optional)"
                value={buyDetails.message}
                onChange={(e) => setBuyDetails({ ...buyDetails, message: e.target.value })}
              />
              <button type="submit" style={{ ...predictBtn, background: "#3b82f6", width: "100%" }} disabled={buyStatus === "loading"}>
                {buyStatus === "loading" ? "Sending..." : "Send Buy Request"}
              </button>
              {buyStatus === "success" && <p style={{ color: "#34d399", textAlign: "center" }}>✅ Buy request sent! Seller will contact you soon.</p>}
              {buyStatus === "error" && <p style={{ color: "#f87171", textAlign: "center" }}>❌ Failed to send request. Try again.</p>}
            </form>
            <button onClick={sendWhatsAppMessage} style={{ ...predictBtn, background: "#25d366", width: "100%", marginTop: "10px" }} disabled={!buyDetails.name || !buyDetails.phone}>
...
// ContactSection & TeamSection (SAME AS BEFORE - no changes)
const ContactSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        const { error } = await response.json();
        throw new Error(error || "Contact submit failed");
      }
      ...
      console.error(err);
      setStatus("error");
      setErrorMsg("Server error while submitting contact message.");
      setTimeout(() => setStatus(""), 4000);
    }
  };
    ...
    <section style={{background:"rgba(2,6,23,0.95)", padding:"100px 8%", color:"white", textAlign:"center"}}>
      ...
      <div className="team-grid">
        {[
          ...
          { initials: "KHSV", name: "KORASEEKA HARI SAI VENKAT", role: "💻 Full Stack" },
          { initials: "KD", name: "KOLLURU DHANUSH", role: "📊 Data Analyst" },
          { initials: "VS", name: "VADICHARLA SHIVA", role: "🎨 UI/UX Designer" }
        ].map((member, i) => (
...
// STYLES (Updated)
const pageStyle = {minHeight:"100vh", background:"linear-gradient(135deg, #0f172a 0%, #1e293b 100%)", display:"flex", justifyContent:"center", alignItems:"center", padding:"20px"};
const formContainer = {background:"rgba(15,23,42,0.98)", padding:"50px", borderRadius:"30px", maxWidth:"600px", width:"100%", border:"1px solid rgba(255,255,255,0.1)", boxShadow:"0 30px 60px rgba(0,0,0,0.5)"};
const inputStyle = {padding:"18px", borderRadius:"12px", border:"2px solid rgba(255,255,255,0.2)", background:"rgba(15,23,42,0.9)", color:"white", fontSize:"16px", width:"100%"};
const titleStyle = {color:"#60a5fa", textAlign:"center", marginBottom:"30px", fontSize:"28px"};
const formStyle = {display:"flex", flexDirection:"column", gap:"20px"};
const predictBtn = {padding:"20px", background:"linear-gradient(135deg, #7c3aed, #8b5cf6)", border:"none", borderRadius:"12px", color:"white", fontSize:"16px", fontWeight:"700", cursor:"pointer", width:"100%"};
const backBtn = {padding:"18px", background:"linear-gradient(135deg, #475569, #64748b)", border:"none", borderRadius:"12px", color:"white", fontSize:"16px", fontWeight:"700", cursor:"pointer", width:"100%", marginTop:"20px"};
const predictionStyle = {background:"linear-gradient(135deg, #10b981, #059669)", padding:"30px", borderRadius:"20px", textAlign:"center", color:"white", fontSize:"24px", fontWeight:"700", margin:"20px 0"};
const successStyle = {background:"linear-gradient(135deg, #10b981, #059669)", padding:"50px", borderRadius:"20px", textAlign:"center", color:"white", fontSize:"28px", fontWeight:"800"};
const sectionTitle = {fontSize:"40px", color:"#60a5fa", marginBottom:"40px", fontWeight:"800"};
const mentorImg = {width:"220px", height:"220px", borderRadius:"50%", objectFit:"cover", border:"6px solid rgba(56,189,248,0.5)", marginBottom:"25px"};
    ...
    <div style={pageStyle}>
      <div style={formContainer}>
        ...
        <form onSubmit={handleSubmit} style={formStyle}>
          ...
          {status === "success" && <p style={{ color: "#34d399", textAlign: "center" }}>Login successful ✅ Redirecting...</p>}
          {status === "error" && <p style={{ color: "#f87171", textAlign: "center" }}>Invalid credentials ❌</p>}
        </form>
        <p style={{ textAlign: "center", color: "rgba(255,255,255,0.6)", marginTop: "15px" }}>
